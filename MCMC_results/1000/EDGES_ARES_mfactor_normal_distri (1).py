...
start_guess = {'multiplication_factor': 5, 'fstar': 0.03, 'fX': 0.9, 'fesc':0.1, 'clumping_factor':0.1, 'cX' : 4.89} #Do not change the key to multplication factor or change the run_ares function respectively
...
def mcmc (max_steps, start_guess):
    ...
    for i in range(1, max_steps):
        ...
        while (True): # to chack if  all the parameters are in the reasonable limit
            new_param = normal(chain[i-1, :], 0.005) #new random point in the parameter space
...
param = open('mcmc_params.txt','w')
param.write(repr(params))
param.close()
